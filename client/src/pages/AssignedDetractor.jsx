...
const AssignedDetractor = () => {
  ...
  useEffect(() => {
    const fetchAllTasks = async () => {
      try {
        const { data } = await api.get("/tasks/get-detractor-assigned-tasks", {
          headers: { Authorization: `Bearer ${localStorage.getItem("accessToken")}` },
        });
    ...
    fetchAllTasks();
  }, [user, updateStateDuringSave, updateRefetchTasks]);

  const TASKS_PER_PAGE = 5;

  const fetchTasks = async ({ pageParam = 1 }) => {
    const { data } = await api.get(`/tasks/get-paginated-detractor-assigned-tasks?page=${pageParam}`, {
      headers: { Authorization: `Bearer ${localStorage.getItem("accessToken")}` },
    });
    return data;
  };
  ...
  const { data, status, error, fetchNextPage, isFetchingNextPage, hasNextPage } = useInfiniteQuery({
    queryKey: ['assigned-to-me-detractor-tasks'],
    queryFn: fetchTasks,
    initialPageParam: 1,
      ...
      {selected === 0 ? (
        <Box>
          {filteredTasks.length === 0 ? (
            <Typography align="center" sx={{ mt: 3, color: "antiquewhite" }}>
              No tasks found.
            </Typography>
          ) : (
            <>
              ...
              <Box sx={{ my: 2, display: 'flex', justifyContent: 'center' }} ref={ref}>
                {isFetchingNextPage ? (
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: "30px" }}></Box>
                )}
              </Box>
            </>
          )}
...
export default AssignedDetractor;