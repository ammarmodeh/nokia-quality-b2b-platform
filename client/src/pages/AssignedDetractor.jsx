...
const AssignedDetractor = () => {
  ...
  useEffect(() => {
    const fetchAllTasks = async () => {
      try {
        const { data } = await api.get(`/tasks/get-detractor-assigned-tasks?validationStatus=${validationFilter}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("accessToken")}` },
        });
  ...
  const fetchTasks = async ({ pageParam = 1 }) => {
    try {
      const { data } = await api.get(`/tasks/get-paginated-detractor-assigned-tasks?page=${pageParam}&validationStatus=${validationFilter}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("accessToken")}` },
      });
      return data;
    } catch (error) {
  ...
  const { data, status, error, fetchNextPage, isFetchingNextPage, hasNextPage } = useInfiniteQuery({
    queryKey: ['assigned-to-me-detractor-tasks', validationFilter],
    queryFn: fetchTasks,
    initialPageParam: 1,
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems={{ xs: 'start', md: 'center' }} mb={4} spacing={2}>
        <Box>
          ...
          <Typography variant="body2" color="textSecondary">
            Focus on resolving tasks with detractor status.
          </Typography>
        </Box>
