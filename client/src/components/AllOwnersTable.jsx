...
export const AllOwnersTable = ({ tasks }) => {
  ...
  // Get the total violations for each owner
  const ownerViolations = getOwnerViolations(tasks);

  // Sort the ownerViolations array in descending order based on total violations
  const sortedOwnerViolations = ownerViolations.sort((a, b) => b.total - a.total);

  // Prepare rows for the DataGrid
  const rows = sortedOwnerViolations.map((violation, index) => ({
    id: index + 1,
  ...
  return (
    <Stack justifyContent={"center"} sx={{ width: "100%" }}>
      ...
      >
        ...
        >
          Owner Overview ( <span style={{ color: "#0ea5e9" }}>{netTotal}</span> )
        </Typography>
      ...
      <Paper sx={{
        ...
        <DataGrid
          ...
          sx={{
            ...
            "& .MuiDataGrid-overlay": { color: "#64748b" },
            ...
            "& .MuiDataGrid-row": { "&:hover": { backgroundColor: "rgba(100, 116, 139, 0.08)" } },
            ...
            "& .MuiDataGrid-toolbarContainer": {
              ...
              borderBottom: "1px solid #e2e8f0",
              gap: 2,
              "& .MuiButton-root": { color: "#64748b", fontSize: "0.80rem" }
            }
          }}
        />
      </Paper>
      ...
      >
        <DialogTitle sx={{
          ...
          <IconButton
            ...
            sx={{ color: '#ffffff', '&:hover': { backgroundColor: '#2a2a2a' } }}
          >
            <MdClose />
          </IconButton>
        </DialogTitle>
        ...
        <DialogContent dividers sx={{ backgroundColor: '#2d2d2d', color: '#ffffff', padding: '20px 24px' }}>
          <Stack spacing={3}>
        ...
        <DialogActions sx={{ backgroundColor: '#2d2d2d', borderTop: '1px solid #e5e7eb', padding: '12px 24px' }}>
          <Button
            onClick={exportOwnerTasksToExcel}
            variant="contained"
            startIcon={<MdFileDownload />}
            sx={{ backgroundColor: '#1d4ed8', color: '#ffffff' }}
          >
            Export to Excel
          </Button>
          <Button onClick={() => setDialogOpen(false)} sx={{ color: '#ffffff' }}>Close</Button>
        </DialogActions>
      </Dialog>
    </Stack>
  );
