...
const EditSessionDialog = ({ open, onClose, session, onSave }) => {
  ...
  useEffect(() => {
    if (session) {
      ...
      // Handle conductedBy migration
      if (session.conductedBy) {
        if (Array.isArray(session.conductedBy)) {
      ...
      // Parse Duration
      if (session.duration) {
        const hoursMatch = session.duration.match(/(\d+)\s*hr/);
        const minutesMatch = session.duration.match(/(\d+)\s*min/);
      ...
      // Handle both old string format and new array format for outlines
      if (session.outlines) {
        if (typeof session.outlines === 'string') {
          // Convert old string format to new structure
          // Split by common delimiters and create a single main topic
          const lines = session.outlines.split(/[,\n•\-]/).filter(line => line.trim());
          if (lines.length > 0) {
            setOutlines([{
              mainTopic: "Training Topics",
              subTopics: lines.map(line => line.trim())
            }]);
          }
  ...
  const handleFieldChange = (field) => (e) => {
    ...
    switch (field) {
      ...
      // Duration handled separately due to split fields
    }
  };
