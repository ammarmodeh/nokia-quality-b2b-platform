...
import { ArrowBack, BarChart, PictureAsPdf } from '@mui/icons-material';
  ...
  const handleExportToPDF = async () => {
    try {
      ...
      if (categoryChartRef.current) {
        ...
        categoryChartImage = canvas.toDataURL("image/png");
      }

      // 2. Analyze Performance for Text Content
      ...
      const auditLogs = assessment.checkPoints
        .map((p, i) => `| ${i + 1}. ${p.name} | ${p.score} | ${p.category} | ${p.notes ? p.notes.replace(/\n/g, ' ') : '-'} |`)
        .join('\n');

      const strengthsText = strengths.map(s => `* **${s.category}** (${s.averageScore}%): ${s.checkPoints.map(cp => cp.name).join(', ')}`).join('\n');
...
### CheckPoint Scores Overview
${chartImage ? `![CheckPoint Chart](${chartImage})` : "_Visual not available_"}

### Category Scores Overview
${categoryChartImage ? `![Category Chart](${categoryChartImage})` : "_Visual not available_"}

## 🧠 Performance Analysis

### ✅ Strongest Areas
${strengthsText || "No standout strengths identified."}

### ⚠️ Growth Opportunities
${improvementsText || "No priority growth areas identified."}

## 📋 Detailed Audit Logs
| ID | CheckPoint | Score | Category | Observations |
| :--- | :--- | :---: | :--- | :--- |
${auditLogs}

## ✍️ Auditor Feedback
${assessment.feedback || "No additional feedback provided."}

---
*Generated by Reach Quality Assessors*
`;

      const reportTitle = `${teamName.replace(/\s+/g, '_')}_Practical_Assessment_${new Date(assessment.assessmentDate).toISOString().split('T')[0]}`;

      const response = await api.post('/ai/report/download', {
        reportContent: reportMarkdown,
        title: `Practical Assessment: ${teamName}`,
        format: 'pdf'
      }, {
        responseType: 'blob'
      });
