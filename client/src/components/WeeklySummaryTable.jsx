...
const WeeklySummaryTable = ({ tasks, fieldTeams = [] }) => {
  ...
  const exportToExcel = () => {
    ...
    rows.forEach(row => {
      // Base summary data that repeats for every task of this team
      const teamSummary = {
    ...
    const fileName = `Weekly_Summary_${periodLabel}_${new Date().toISOString().slice(0, 10)}.xlsx`;
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2, flexDirection: isMobile ? 'column' : 'row' }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, width: isMobile ? '100%' : 'auto', flexWrap: 'wrap' }}>
          ...
          <FormControlLabel
            ...
            sx={{ color: '#fff', whiteSpace: 'nowrap', width: isMobile ? '100%' : 'auto', m: 0 }}
          />
          ...
          {/* <AIAnalysisButton
            data={rows}
            title={`Weekly Summary - ${selectedWeek ? availableWeeks.find(w => w.key === selectedWeek)?.label : 'Custom Range'}`}
            context={`Detailed weekly performance analysis for ${rows.length} teams. Period: ${selectedWeek ? availableWeeks.find(w => w.key === selectedWeek)?.label : 'Custom Range'}`}
          /> */}
        </Box>
      </Box>
...
export default React.memo(WeeklySummaryTable);
