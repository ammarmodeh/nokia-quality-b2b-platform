...
const migrate = async () => {
    try {
        ...
        console.log("Connected to MongoDB for v4 flat migration...");

        // Clear existing tasks and tickets
        ...
        // Cache field teams for lookup
        const teams = await FieldTeamsSchema.find({});
        const teamMap = new Map();
        teams.forEach(team => {
            teamMap.set(team.teamName.trim().toLowerCase(), team._id);
        });
        console.log(`Cached ${teamMap.size} field teams for mapping.`);
        ...
        for (const taskData of tasks) {
            try {
                ...
                let teamId = extractId(taskData.teamId);
                if (!teamId && taskData.teamName) {
                    teamId = teamMap.get(taskData.teamName.trim().toLowerCase()) || null;
                }
                ...
                const task = new TaskSchema({
                    ...
                    subtaskType: taskData.subtaskType || null,
